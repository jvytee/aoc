...
fn part_one(input: impl Iterator<Item = String>) -> i32 {
    input.fold((Vec::new(), Vec::new(), 0), part_numbers).2
}

fn part_numbers(
    (pos_num_prev, symbols_prev, acc): (Vec<(usize, i32)>, Vec<usize>, i32),
    line: String,
) -> (Vec<(usize, i32)>, Vec<usize>, i32) {
    let (pos_num, _) = line
